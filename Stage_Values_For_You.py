...
def run_training(
    ...
    if rg_df.empty:
        raise RuntimeError(
            "No RiverGages data collected. Cannot train QAQC model.")

    rg_ids = [c.replace("stage_", "")
              for c in rg_df.columns if c.startswith("stage_")]
    ...
    rg_client = RiverGagesClient()
    rg_miles: Dict[str, float] = {}
    # lat/lon per station for datum conversion
    ...
    if lag_table_path.exists():
        ...
        lag_table = {}
        q_cond = next(
            (usgs_df[c] for c in usgs_df.columns if usgs_df[c].notna().sum() > 0), None)
        for up_sid, dn_sid in pairs:
            pair_key = f"{up_sid}_to_{dn_sid}"
            prog_lags.update(0, item_name=pair_key)
            try:
                lag_info = calibrate_pair_lags(rg_df[f"stage_{up_sid}"].dropna(
                ), rg_df[f"stage_{dn_sid}"].dropna(), q_series=q_cond)
                lag_table[pair_key] = lag_info
            except Exception as exc:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train.add_argument("--freq", type=str, default=DEFAULT_FREQ,
                       help="Resampling frequency (default D)")
