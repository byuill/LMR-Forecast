...
USGS_BASE = "https://waterservices.usgs.gov/nwis/dv/"
...
DEFAULT_SOURCE_DATUM = "NAVD88"   # assumed vertical datum for RiverGages stage data
...
PEAK_HALF_WINDOW_DAYS = 15              # days each side for local-extremum detection
...
class ProgressTracker:
	...
	def update(self, step: int = 1, item_name: str = "") -> None:
		...
		sys.stdout.write(f"\r{self.description} |{bar}| {percent:3d}% [{self.current}/{self.total}] ETA: {self.format_time(eta_sec)} | {item_name[:15].ljust(15)}")
		sys.stdout.flush()

	def finish(self) -> None:
		if self.total == 0: return
		sys.stdout.write(f"\r{self.description} |{'█' * 25}| 100% [{self.total}/{self.total}] Time: {self.format_time(time.time() - self.start_time)}".ljust(85) + "\n")
		sys.stdout.flush()
	...
	def data(self) -> pd.DataFrame:
		if self._df is None:
			if not self.csv_path.exists():
				...
				self._df = self._df.dropna(subset=["MILE", "LATITUDE1", "LONGITUDE1"]).sort_values("MILE").reset_index(drop=True)
		return self._df
...
class RiverGagesClient:
	...
	def _extract_timeseries(xml_text: str) -> pd.DataFrame:
		...
		for match in pattern.finditer(xml_text):
			...
			try:
				...
				continue
			timestamps.append(pd.Timestamp(ts).tz_localize(None) if getattr(ts, "tzinfo", None) else pd.Timestamp(ts))
			values.append(val)
	...
	def get_station_metadata(self, site: str) -> dict:
		try:
			raw = self._request_raw("GetSiteInfo", site, TARGET_STAGE_VARIABLE, "2023-01-01T00:00", "2023-01-02T00:00")
			lat_m = re.search(r'<latitude>([^<]+)</latitude>', raw, re.IGNORECASE)
			lon_m = re.search(r'<longitude>([^<]+)</longitude>', raw, re.IGNORECASE)
			if lat_m and lon_m:
	...
	def fetch_stage(
		...
		for chunk_start, chunk_end in date_chunks(start, end):
			...
			try:
				...
				log.warning("RiverGages fetch failed for site=%s chunk=%s..%s: %s", site, b, e, exc)
...
class USGSClient:
	@staticmethod
	def fetch_discharge_daily(station_id: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
		cache_name = f"usgs_q_{station_id}_{dt_to_str(start)}_{dt_to_str(end)}.csv"
		cache_path = CACHE_DIR / cache_name
		if cache_path.exists():
		...
		params = {
			"format": "json",
			"sites": station_id,
			"startDT": dt_to_str(start),
			"endDT": dt_to_str(end),
			"parameterCd": USGS_DISCHARGE_PARAM,
			"siteStatus": "all",
		}
		r = requests.get(USGS_BASE, params=params, timeout=REQUEST_TIMEOUT)
		r.raise_for_status()
		payload = r.json()
			...
			for group in values:
				for item in group.get("value", []):
					...
					try:
						...
						continue
					rows.append((pd.Timestamp(dt).normalize(), fv))

		if not rows:
			return pd.Series(dtype=float, name=f"q_{station_id}")

		rdf = pd.DataFrame(rows, columns=["date", "value"]).drop_duplicates("date").sort_values("date")
...
class NOAAClient:
	@staticmethod
	def fetch_water_level_daily(station_id: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
		cache_name = f"noaa_wl_{station_id}_{dt_to_str(start)}_{dt_to_str(end)}.csv"
		cache_path = CACHE_DIR / cache_name
		if cache_path.exists():
			cdf = pd.read_csv(cache_path, parse_dates=["date"])
		...
		for chunk_start, chunk_end in date_chunks(start, end, chunk_days=31):
			...
			try:
				r = requests.get(NOAA_BASE, params=params, timeout=REQUEST_TIMEOUT)
				r.raise_for_status()
				...
				log.warning("NOAA fetch failed station=%s chunk=%s..%s: %s", station_id, chunk_start, chunk_end, exc)
	...
	def _read_station_record(record_path: Path) -> Optional[Tuple[str, List[dict]]]:
		...
		for line in text.splitlines():
			...
			if stripped.startswith("#"):
				...
				m = re.match(r'#\s*USACE Code\s*:\s*(\d{5})', stripped, re.IGNORECASE)
				if m:
					sid = m.group(1)
				continue
		...
		for page in pages:
			for table in (page.extract_tables() or []):
				...
				for row in table:
					...
					start_dt = pd.to_datetime(cells[3], errors="coerce") if cells[3] else pd.NaT
					if pd.isna(start_dt):
						continue
		...
		for pdf_path in pdfs:
			...
			try:
				with pdfplumber.open(pdf_path) as pdf:
					...
					if sid is None:
						log.debug("No USACE code found in %s — skipping", pdf_path.name)
						prog.update(1, item_name=pdf_path.stem[:15])
						continue
			...
			self._write_station_record(record_path, pdf_path.name, sid, adj_rows)
			new_parsed += 1
		...
		for row in applicable_sorted:
			...
			if not pd.isna(end):
				...
				if span < min_span_days:
					continue
				mask = (date_index >= start) & (date_index <= end) & (~assigned)
			else:
	...
	def apply_to_dataframe(
		...
		for pdf_sid, adj_rows in adj_data.items():
			...
			if col not in corrected.columns:
				continue
			offsets = self.build_offset_series(adj_rows, corrected.index, target_datum)
...
class DatumConverter:
	...
	def _load_cache(self) -> None:
		if VDATUM_OFFSET_CACHE.exists():
			try:
				...
				log.warning("VDatum offset cache unreadable, starting fresh: %s", exc)
				self._cache = {}
	...
	def get_offset_ft(
		...
		offset_m = float(result["t_z"])   # t_z for s_z=0 is the datum separation
	...
	def convert_series(
		...
		offset_ft = self.get_offset_ft(sid, lat, lon, s_v_frame, t_v_frame, region)
	...
	def convert_network(
		...
		for col in stage_cols:
			...
			if "lat" not in coords or "lon" not in coords:
				log.warning("No coordinates for station %s — datum conversion skipped.", sid)
...
def load_rg_station_ids() -> List[str]:
	...
	lines = [x.strip() for x in RG_STATIONS_FILE.read_text(encoding="utf-8").splitlines() if x.strip()]
	if not lines:
		return []
...
def find_peaks_troughs(
	...
	for i in range(half_window, n - half_window):
		...
		neighbours = np.concatenate([vals[i - half_window: i], vals[i + 1: i + half_window + 1]])
...
def calibrate_pair_lags(
	...
	for t_up in extrema_up:
		...
		if q_series is not None:
			...
			matched_q.append(float(q_window.mean()) if len(q_window) > 0 else np.nan)
		else:
			matched_q.append(np.nan)

	if len(matched_lags) < 5:
		log.debug("  Peak matching: only %d pairs — using xcorr=%d", len(matched_lags), xc)
		return result
	...
	if q_series is not None and q_valid_mask.sum() >= 8:
		...
		for i, label in enumerate(DISCHARGE_BIN_LABELS):
			mask = (q_arr >= q_cuts[i]) & (q_arr <= q_cuts[i + 1]) & q_valid_mask
			if mask.sum() >= 3:
...
def calibrate_network_lags(
	...
	if len(ordered) < 2:
		log.warning("Fewer than 2 stations with river miles — skipping lag calibration.")
		return {}
...
def make_station_feature_table(
	...
	for lag in [1, 2, 3, 5, 7]:
		...
		new_cols[col_name] = target_series.shift(lag)
		feature_cols.append(col_name)

	_d1 = target_series.diff(1)
	_d3 = target_series.diff(3)
	_d1r7 = _d1.rolling(7, min_periods=3).mean()
	...
	if q_cols:
		q_composite = pd.concat(
			[base_df[c] / max(float(base_df[c].median()), 1.0) for c in q_cols],
			axis=1,
		).mean(axis=1)
	else:
	...
	for sid in rg_station_ids:
		...
		if cal:
			...
			lag_low_q = max(0, int(round(dist / (FLOW_SPEED_BASE_MI_PER_DAY * 1.4))))
			lag_high_q = max(0, int(round(dist / (FLOW_SPEED_BASE_MI_PER_DAY * 0.6))))
		...
		for lag in lags_to_use:
			feat = f"{base_name}_lag{lag}"
			if feat not in new_cols:
				new_cols[feat] = neighbor_series.shift(lag)
				feature_cols.append(feat)

		# Rate-of-change at the central lag
		d1_feat = f"{base_name}_d1_lag{lag_center}"
		if d1_feat not in new_cols:
			new_cols[d1_feat] = neighbor_series.diff(1).shift(lag_center)
		...
		if dist is not None and dist > 0:
			...
			speed_series = FLOW_SPEED_BASE_MI_PER_DAY * (q_safe ** FLOW_SPEED_EXPONENT)
			exp_lag_feat = f"expected_lag_{sid}_days"
			if exp_lag_feat not in new_cols:
				new_cols[exp_lag_feat] = (dist / speed_series).clip(lower=0, upper=30).round()
	...
	for c in q_cols:
		for lag in [0, 1, 2, 3, 5, 7]:
			...
			if qcol not in new_cols:
				new_cols[qcol] = base_df[c].shift(lag)
		...
		if c in base_df.columns:
			for lag in [0, 1, 2, 3]:
				...
				if wcol not in new_cols:
					new_cols[wcol] = base_df[c].shift(lag)
					feature_cols.append(wcol)

	for lag in [0, 1, 2, 3]:
		wcol = f"q_composite_lag{lag}"
		if wcol not in new_cols:
			new_cols[wcol] = q_composite.shift(lag)
			feature_cols.append(wcol)
	...
	table = table[feature_cols + [target_col]].replace([np.inf, -np.inf], np.nan)
	return table
...
def fit_iterative_qaqc(
	...
	if usable.empty:
		raise ValueError("No trainable rows: target column has no valid observations")
	...
	for _ in range(max_iter):
		...
		pred_all = pd.Series(model.predict(usable[feature_columns]), index=usable.index)
		resid = usable[target_col] - pred_all
		sigma = robust_sigma(resid.values)
	...
	sigma = robust_sigma((obs_final - pred_final).values)
	...
	result = pd.DataFrame(index=usable.index)
	result["observed"] = obs_final
	result["predicted"] = pred_final
	result["residual"] = obs_final - pred_final
	result["is_outlier"] = flagged
	result["corrected"] = np.where(flagged, pred_final, obs_final)
...
# ---------------------------------------------------------------------------
def collect_historical_data(start: pd.Timestamp, end: pd.Timestamp, freq: str = DEFAULT_FREQ) -> Dict[str, pd.DataFrame]:
	...
	for sid in rg_ids:
		...
		s = rg_client.fetch_stage(sid, start, end, variable=TARGET_STAGE_VARIABLE)
		if not s.empty:
			s = s.sort_index().resample(freq).mean()
	...
	for sid in usgs_ids:
		...
		try:
			s = usgs_client.fetch_discharge_daily(sid, start, end)
			if not s.empty:
				usgs_series[f"q_{sid}"] = s.sort_index().resample(freq).mean()
		except Exception as exc:
	...
	for label, sid in NOAA_STATIONS.items():
		...
		try:
			s = noaa_client.fetch_water_level_daily(sid, start, end)
			if not s.empty:
				noaa_series[f"wl_{sid}"] = s.sort_index().resample(freq).mean()
		except Exception as exc:
...
def run_training(
	...
	history = collect_historical_data(start, end, freq=freq)
	...
	rg_ids = [c.replace("stage_", "") for c in rg_df.columns if c.startswith("stage_")]
	...
	log.info("--- MILESTONE 2: Building Base Frame & Spatial Mapping ---")
	mapper = RiverMileMapper()
	rg_client = RiverGagesClient()
	rg_miles: Dict[str, float] = {}
	rg_coords: Dict[str, Dict[str, float]] = {}  # lat/lon per station for datum conversion
	...
	if lag_table_path.exists():
		...
		pairs = []
		ordered = sorted([(sid, mile) for sid, mile in rg_miles.items() if f"stage_{sid}" in rg_df.columns], key=lambda x: x[1], reverse=True)
		for i, (up_sid, up_mile) in enumerate(ordered):
			for j in range(i + 1, min(i + 4, len(ordered))):
				dn_sid, dn_mile = ordered[j]
		...
		lag_table = {}
		q_cond = next((usgs_df[c] for c in usgs_df.columns if usgs_df[c].notna().sum() > 0), None)
		for up_sid, dn_sid in pairs:
			pair_key = f"{up_sid}_to_{dn_sid}"
			prog_lags.update(0, item_name=pair_key)
			try:
				lag_info = calibrate_pair_lags(rg_df[f"stage_{up_sid}"].dropna(), rg_df[f"stage_{dn_sid}"].dropna(), q_series=q_cond)
				lag_table[pair_key] = lag_info
			except Exception as exc:
				log.warning("\nLag calibration failed for %s: %s", pair_key, exc)
	...
	if checkpoint_path.exists():
		log.info("--- MILESTONE 4: Resuming Iterative Model Training from Checkpoint ---")
	...
	for sid in rg_ids:
		...
		table = make_station_feature_table(base, sid, rg_ids, rg_miles, lag_table)
		...
		bundles[sid] = bundle
		corrected_network[f"stage_{sid}"] = result["corrected"].reindex(base.index)
		predicted_network[f"stage_{sid}"] = result["predicted"].reindex(base.index)
		outlier_network[f"stage_{sid}"] = result["is_outlier"].reindex(base.index).fillna(False)

		row = {"station_id": sid}
		row.update(bundle.metrics)
		metrics_rows.append(row)

		result_out = OUTPUT_DIR / f"station_{sanitize_station_id(sid)}_qaqc_timeseries.csv"
	...
	rg_df.reset_index(names="date").to_csv(OUTPUT_DIR / "raw_stage_network.csv", index=False)
	corrected_network.reset_index(names="date").to_csv(OUTPUT_DIR / "corrected_stage_network.csv", index=False)
	predicted_network.reset_index(names="date").to_csv(OUTPUT_DIR / "predicted_stage_network.csv", index=False)
	outlier_network.reset_index(names="date").to_csv(OUTPUT_DIR / "outlier_flags_network.csv", index=False)
...
def load_trained_outputs() -> Tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
	...
	corrected = pd.read_csv(OUTPUT_DIR / "corrected_stage_network.csv", parse_dates=["date"]).set_index("date")
	predicted = pd.read_csv(OUTPUT_DIR / "predicted_stage_network.csv", parse_dates=["date"]).set_index("date")
	raw = pd.read_csv(OUTPUT_DIR / "raw_stage_network.csv", parse_dates=["date"]).set_index("date")
...
def query_stage_series(
	...
	col = f"stage_{target}"
	_ref_sid: Optional[str] = None   # station whose coords guide datum conversion
	...
	if col in window.columns:
		...
		try:
			...
			available = ", ".join(sorted([c.replace("stage_", "") for c in df.columns if c.startswith("stage_")]))
			raise KeyError(f"'{target}' is not a valid station ID or River Mile. Available stations: {available}")

		rg_miles = artifact.get("rg_miles", {})
		if not rg_miles:
			raise ValueError("No River Mile mappings found in artifact. Cannot interpolate.")

		stations_with_miles = [(sid, mile) for sid, mile in rg_miles.items() if f"stage_{sid}" in window.columns]
		...
		if not below:
			...
			if lo_sid == hi_sid:
				...
				interp_series = window[f"stage_{lo_sid}"] * (1 - frac) + window[f"stage_{hi_sid}"] * frac
	...
	if effective_t and effective_t != effective_s and _ref_sid is not None:
		...
		if coords and "lat" in coords and "lon" in coords:
			...
			log.info("Output datum: %s (converted from %s)", effective_t, effective_s)
		else:
			log.warning(
...
def run_dashboard() -> None:
	...
	while True:
		...
		source = input("Source [corrected/predicted/raw] (default corrected): ").strip() or "corrected"
		s_datum = input(f"Source datum (default {DEFAULT_SOURCE_DATUM}): ").strip() or DEFAULT_SOURCE_DATUM
		t_datum = input("Target datum (leave blank to keep source datum): ").strip()
		try:
			...
			if t_datum and t_datum.upper() != s_datum.upper():
				print(f"Vertical datum: {t_datum.upper()} (converted from {s_datum.upper()})")
		except Exception as exc:
...
def build_parser() -> argparse.ArgumentParser:
	...
	train = sub.add_parser("train", help="Train QAQC model and generate corrected outputs")
	train.add_argument("--years", type=int, default=DEFAULT_LOOKBACK_YEARS, help="Historical lookback window")
	train.add_argument("--freq", type=str, default=DEFAULT_FREQ, help="Resampling frequency (default D)")
	train.add_argument("--z-thresh", type=float, default=OUTLIER_Z, help="Outlier residual threshold in robust sigma")
	train.add_argument("--max-iter", type=int, default=ITER_MAX, help="Max iterative outlier-removal rounds")

	query = sub.add_parser("query", help="Query trained series for station/date interval")
	query.add_argument("--target", required=True, help="RiverGages station ID or LMR River Mile")
	query.add_argument("--start", required=True, help="YYYY-MM-DD")
	query.add_argument("--end", required=True, help="YYYY-MM-DD")
	query.add_argument("--source", default="corrected", choices=["raw", "corrected", "predicted"])
